...
ConversionPatternRewriterImpl::lookupOrDefault(Value from,
                                               TypeRange desiredTypes) const {
  // If pattern rollback is allowed, use the mapping.
  if (config.allowPatternRollback)
    return mapping.lookupOrDefault(from, desiredTypes);

  // Helper function: If the given value is defined by an unresolved
  // materialization, return the unrealized_conversion_cast op.
  auto getCastOp = [&](Value v) -> UnrealizedConversionCastOp {
    auto castOp = v.getDefiningOp<UnrealizedConversionCastOp>();
    if (!castOp)
      return {};
    // User-created unrealized_conversion_cast ops are not unresolved
    // materializations.
  ...
  // Trace back the chain of unrealized_conversion_cast ops to find values with
  // the desired types.
  //
  // Note: In "no rollback" mode, IR changes are materialized immediately.
  // We do not have to worry about looking up a value that is going to be
  // replaced/erased. We only care about materializations here.
  //
  // In "no rollback" mode, materializations form a tree instead of a chain.
  // To avoid an overly expensive lookup, we do not consider materializations
  // in sibling branches of the tree. This means that we sometimes create
  // duplicate materializations. This lookup could be optimized in the future.
  //
  // TODO: This lookup supports only 1:1 materializations.
  Value next = from;
  while (desiredTypes != TypeRange(next.getType())) {
    auto castOp = getCastOp(next);
    // Stop early if a value with the desired type is found. Otherwise, the
    // lookup ends with the most recent replacement value.
